...
func GetCodeOwners(sourceRoot string) *CodeOwners {
	paths := []string{
		filepath.Join(sourceRoot, "CODEOWNERS"),
		filepath.Join(sourceRoot, ".github", "CODEOWNERS"),
		filepath.Join(sourceRoot, ".gitlab", "CODEOWNERS"),
		filepath.Join(sourceRoot, ".docs", "CODEOWNERS"),
	}

	for _, path := range paths {
		if _, err := os.Stat(path); err == nil {
			codeowners, cErr := NewCodeOwners(path)
			if cErr == nil {
				return codeowners
			}
		}
